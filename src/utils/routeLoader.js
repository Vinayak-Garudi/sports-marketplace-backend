...
const autoLoadRoutes = (app) => {
    ...
    try {
      ...
      if (routeConfig.rateLimit) {
        const moduleRateLimit = rateLimit({
          ...
          message: routeConfig.rateLimit.message || `Too many requests to ${moduleName} API`
        });
        app.use(routeConfig.path, moduleRateLimit);
        console.log(`🛡️  Rate limiting applied to ${moduleName}: ${routeConfig.rateLimit.max} requests per ${routeConfig.rateLimit.windowMs/1000/60} minutes`);
      }
      ...
      if (routeConfig.middleware && Array.isArray(routeConfig.middleware)) {
        ...
        console.log(`🔒 Applied ${routeConfig.middleware.length} middleware(s) to ${moduleName}`);
      }
      ...
      console.log(`✅ Loaded module: ${moduleName} -> \x1b[32m${routeConfig.path}\x1b[0m`);
  ...
  if (loadedRoutes.length > 0) {
    ...
    console.log('\x1b[36m│\x1b[0m \x1b[1mModule\x1b[0m          \x1b[36m│\x1b[0m \x1b[1mRoute\x1b[0m                      \x1b[36m│\x1b[0m \x1b[1mDescription\x1b[0m                         \x1b[36m│\x1b[0m \x1b[1mVersion\x1b[0m \x1b[36m│\x1b[0m');
    console.log('\x1b[36m├─────────────────┼────────────────────────────┼─────────────────────────────────────┼─────────┤\x1b[0m');
    
    loadedRoutes.forEach((route, index) => {
      const module = route.module.padEnd(15);
      const path = route.path.padEnd(26);
      const description = route.description.length > 35 ? 
        route.description.substring(0, 32) + '...' : 
        route.description.padEnd(35);
      const version = route.version.padEnd(7);
      
      console.log(`\x1b[36m│\x1b[0m ${module} \x1b[36m│\x1b[0m \x1b[32m${path}\x1b[0m \x1b[36m│\x1b[0m ${description} \x1b[36m│\x1b[0m ${version} \x1b[36m│\x1b[0m`);
    });
